...
import OrderColumn from "./OrderColumn"; // See code below
import OrderCard from "./OrderCard";       // See code below
...
const initialColumns = {
    ...
    preparing: [
        { id: "2", customer: "Bob", items: "Widget C", phase: "preparing" },
    ],
    packing: [
        { id: "3", customer: "Charlie", items: "Widget D, Widget E", phase: "packing" },
    ],
    shipped: [
        { id: "4", customer: "Dana", items: "Widget F", phase: "shipped" },
    ],
    delivered: [],
};

const phases: Array<keyof typeof initialColumns> = ["received", "preparing", "packing", "shipped", "delivered"];
...
export default function OrdersPage() {
    ...
    const handleDragEnd = (event: any) => {
        ...
        // If moved within the same column, just reorder
            ...
            <DndContext
                ...
                onDragStart={({ active }) => {
                    const containerId = phases.find(phase =>
                        columns[phase as keyof typeof columns].some(order => order.id === active.id)
                    );
                    setActiveOrder(active.data.current ? { ...active.data.current.item, containerId } : null);
                }}
                onDragEnd={handleDragEnd}
            >
                {phases.map((phase) => (
                    <div key={phase}>
                        <SortableContext items={columns[phase as keyof typeof columns].map((order) => order.id)} strategy={verticalListSortingStrategy}>
                ...
                <DragOverlay>
                    {activeOrder ? (
                        <OrderCard order={activeOrder} dragOverlay />
                    ) : null}
                </DragOverlay>
            </DndContext>
        </div>
